...
# I've also thought it would be important to write a book on [Python](https://www.python.org/) for R users in an easy to follow way. Many of my friends from graduate school and beyond are intamitaly familiar with R, but havn't picked up Python yet. Many resources out there either teach statistics/data science in Python for those whom already know Python (i.e. are familiar with not only the syntax, and pythonic principals, but also oop) or start from scratch assuming no programming knowlege. Even worse, some place an inordinate amount of attention on some packages like numpy or matplotlib, whcih, while powerful, aren't needed to get a grasp on basic or even intermediate data science work in python. 
#
